	...
	try {
		...
		switch (command) {
			...
			case 'ssweb': {
				...
				if (!args[0]) return newReply(
					...
					` ⌬ 📟 tab → Tampilan tablet\n\n` +
					`🔍 *Contoh penggunaan:*\n` +
					`${prefix + command} hp https://www.emanpedia.my.id`
				);
      ...
      case "fesnuk":
			...
			case 'instatv': {
				...
				if (!text) return newReply(`⚠️ Kirim perintah: ${prefix + command} <url>\n\n🤔 Contoh:\n${prefix + command} https://www.instagram.com/reel/DL6gAPLJPN7/?igsh=NWFueXZyMXpmZXB3`);
      ...
      case 'pin' :
       {
			...
			case 'menu': {
				try {
					...
					let samplemenu = `sᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ nara bot 👋 
...
┏ status user
┃ ➣ 💰 sᴀʟᴅᴏ: ${formatAngka(userData.uang || '0')}
┃ ➣ 💳 ʟɪᴍɪᴛ: ${userData.limit || 'Belum ada'}
┃ ➣ 🌟 sᴛᴀᴛᴜs: ${userData.premium ? 'Premium 🔥' : 'Gratisan 😜'}
┗

┏ info bot
┃ ➣  ʀᴜɴᴛɪᴍᴇ: ${runtime(process.uptime())}
┃ ➣  ᴍᴏᴅᴇ: ${sock.public ? 'ᴘᴜʙʟɪᴄ' : `sᴇʟғ`}
┃ ➣  ᴅᴀᴛᴇ: ${date}
┃ ➣  platfrom: ArchLinux
┗
...
`;
			...
			case 'allmenu': {
				...
				let me = m.sender;
				await m.react('⏱️');
				let simplemenu = `sᴇʟᴀᴍᴀᴛ ᴅᴀᴛᴀɴɢ ᴅɪ nara bot 👋 
...
${allMenu(prefix)}`;
        ...
        contextInfo: {
            mentionedJid: [m.sender],
            externalAdReply: {
                thumbnailUrl: 'https://raw.githubusercontent.com/bromanprjkt/botnara/refs/heads/main/3733746f-f520-40f1-9e88-b7330caf1c07.jpeg',
                mediaType: 1,
                renderLargerThumbnail: true
            }
        }
